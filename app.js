...
const main = (redis, iceServers) => {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("register as viewer", async (user) => {
            ...
            const chatData = await redis.zRange(
                `room:${user.room}:chats`,
                0,
                -1,
            );
            const usersOnline = await redis.get(
                `room:${user.room}:online_users`,
            );
            ...
            socket
                .to(broadcasters[user.room])
                .emit("new viewer", user, iceServers);
        });
        ...
        socket.on("offer", (id, event) => {
            ...
            socket
                .to(id)
                .emit("offer", event.broadcaster, event.sdp, iceServers);
        });

        socket.on("answer", (event) => {
            socket
                .to(broadcasters[event.room])
                .emit("answer", socket.id, event.sdp);
        });
        ...
        socket.on("new-message", async (radioId, name, message) => {
            ...
            await redis
                ...
                .then(() =>
                    socket.to(radioId).emit("receive-message", name, message),
                )
                .catch((e) =>
                    console.log("gagal menyimpan data! " + e.toString()),
                );
        });
        ...
        socket.on("disconnect", async () => {
            ...
            if (listenerRoomJoined) {
                ...
                socket
                    .to(listeners[socket.id])
                    .emit("current-listeners", usersOnline);
...
//   method: "PUT",
//   headers: {
//     "Authorization": "Basic " + btoa(`${process.env.ICE_STUN_API_IDENT}:${process.env.ICE_STUN_API_SECRET}`),
